...
- チャンクごとの途中結果保持、失敗チャンクだけの手動再実行
- 接続タイムアウトと応答待ち時間の個別設定
- 一時DOCX/PDFの自動削除

PDFのページ数取得・テキスト抽出には、BSD-3-Clauseライセンスの
[pypdf](https://github.com/py-pdf/pypdf/blob/main/LICENSE)を使用します。
PyMuPDFは使用しません。配布時には依存ライブラリのライセンス表示等の条件も確認してください。

既存の仮想環境を更新する場合は、次を実行します。
...
| `OLLAMA_CONNECT_TIMEOUT_SECONDS` | `10` | サーバへの接続待ち時間（秒） |
| `OLLAMA_READ_TIMEOUT_SECONDS` | `600` | レビュー応答の受信待ち時間（秒） |
...
タイムアウトは画面からも変更できます。モデル一覧取得の応答待ち時間は最大15秒です。
旧設定`OLLAMA_TIMEOUT_SECONDS`は、新しい`OLLAMA_READ_TIMEOUT_SECONDS`が未設定の
場合に限り互換設定として利用します。環境変数を変更した場合はアプリを再起動してください。
応答待ち時間は通信の受信待ちに対する制限であり、処理全体の厳密な時間上限ではありません。
...
各チャンクの処理終了時点で結果をセッションメモリに保存し、処理中も成功結果を表示します。
タイムアウト・通信エラー・不正JSONは失敗チャンクとして記録し、残りのチャンクを続けます。
成功済み結果は失敗があってもダウンロードでき、Markdownには部分結果であることを表示します。
JSONダウンロードには成功済みの指摘だけが含まれます。

失敗時は「再実行する失敗チャンク」で対象を選び、「選択した失敗チャンクだけ再実行」を
押してください。自動リトライは行わず、成功済みチャンクも再送信しません。
タイムアウト設定だけの変更なら成功結果を保持したまま再実行できます。
再実行結果はそのチャンクの前回結果を置き換えます。

文書・対象ページ・レビュー観点・チャンクサイズ・モデル・URLを変更した場合は、結果との
対応が変わるため再実行できません。「レビューを実行」で新規にレビューしてください。
ブラウザの再読み込みによるセッション切断やアプリ再起動では途中結果は失われます。
...
テスト対象は、PDFのページ数取得・指定ページ抽出・エラー処理、ページ範囲検証、プロンプト生成、チャンク分割、JSON解析、Markdown
生成、途中結果保持、失敗チャンク再実行、HTTPタイムアウト分離です。
WordやOllamaの実通信を使わず、通信クライアントをモックして検証します。
...
接続タイムアウトならネットワークやサーバの稼働状態を確認します。
応答待ちタイムアウトなら画面の「応答待ち時間（秒）」または`.env`の
`OLLAMA_READ_TIMEOUT_SECONDS`を増やし、失敗チャンクを手動で再実行します。
タイムアウト後もサーバ側で生成が続いている可能性があるため、即座に繰り返し再送信せず、
サーバの状態を確認してください。チャンク上限を小さくする場合は新規レビューになります。
